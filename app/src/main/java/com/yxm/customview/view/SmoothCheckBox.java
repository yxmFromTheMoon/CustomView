...
public class SmoothCheckBox extends View implements Checkable {
    ...
    private static final int COLOR_TICK      = Color.WHITE; //勾的颜色
    private static final int COLOR_UNCHECKED = Color.WHITE; //UNCHECKED状态颜色
    private static final int COLOR_CHECKED   = Color.parseColor("#FB4846");//CHECK状态颜色
    private static final int COLOR_FLOOR_UNCHECKED = Color.parseColor("#DFDFDF");
    private static final int DEF_ANIM_DURATION = 300;//动画时长
    ...
    private float mLeftLineDistance, mRightLineDistance, mDrewDistance,mOuterBorderWidth;
    ...
    public SmoothCheckBox(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs,0);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public SmoothCheckBox(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttr(AttributeSet attributeSet){
        TypedArray ta = mContext.obtainStyledAttributes(attributeSet, R.styleable.SmoothCheckBox);
        ...
        mOuterBorderWidth = ta.getDimensionPixelSize(R.styleable.SmoothCheckBox_outer_border_width,Utils.dp2px(mContext, DEFAULT_OUTER_BORDER_WIDTH));
        ta.recycle();
        ...
        mFloorPaint.setStrokeWidth(mOuterBorderWidth);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mTickPoints[0].x = Math.round((float) getMeasuredWidth() / 30 * 5);//起点x坐标
        mTickPoints[0].y = Math.round((float) getMeasuredHeight() / 30 * 16);//起点y坐标
        mTickPoints[1].x = Math.round((float) getMeasuredWidth() / 30 * 12);//转折点x坐标
        mTickPoints[1].y = Math.round((float) getMeasuredHeight() / 30 * 23);//转折点y坐标
        mTickPoints[2].x = Math.round((float) getMeasuredWidth() / 30 * 25);//终点x坐标
        mTickPoints[2].y = Math.round((float) getMeasuredHeight() / 30 * 10);//终点y坐标
    ...
    private void drawCenter(Canvas canvas) {
        ...
        float radius = (mCenterPoint.x - mStrokeWidth - mOuterBorderWidth / 2) * mScaleVal;
    ...
    private void drawBorder(Canvas canvas) {
        ...
        int offset = (int) mOuterBorderWidth / 2;
        canvas.drawCircle(mCenterPoint.x, mCenterPoint.y, (radius - offset) * mFloorScale, mFloorPaint);
    }
